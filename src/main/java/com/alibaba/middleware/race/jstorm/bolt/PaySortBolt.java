...
public class PaySortBolt implements   IRichBolt{
	...
	public void execute(Tuple input) {
	    if (input.getSourceStreamId().equals(TopologyUtils.PAY_TOPIC_STREAM)) {
	    	...
	    	//整分时刻13位时间戳
	    	Long minuteTime = (payment.createTime / 1000 / 60) * 60*1000;
	    	//变量初始化
	    	if(lastMinFlag==-1){		
...
//	    	}
	    	...
	    	if(StructUtils.taobaoCacheMap.containsKey(payment.orderId)){
	    		...
	    		if(StructUtils.taobaoCacheMap.get(payment.orderId)<0.001){
	    		...
	    		if(StructUtils.tmallCacheMap.get(payment.orderId)<0.001){
	    			StructUtils.tmallCacheMap.remove(payment.orderId);
	    		}
	    	}else{
	    		System.err.println("pay is faster than order	!!!");
	    	}
	    }
	    collector.ack(input);
	}
