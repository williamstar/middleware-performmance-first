...
//	    		collector.emit(TopologyUtils.TAOBAO_SORT_STREAM,new Values(payment.payAmount,index));
	    		//更新订单总金额,如果全消费完则删除订单
	    		StructUtils.taobaoCacheMap.put(payment.orderId, (Double)StructUtils.taobaoCacheMap.get(payment.orderId)-payment.payAmount);
...
//	    		collector.emit(TopologyUtils.TMALL_SORT_STREAM,new Values(payment.payAmount,index));
	    		//更新订单总金额,如果全消费完则删除订单
	    		StructUtils.tmallCacheMap.put(payment.orderId, (Double)StructUtils.tmallCacheMap.get(payment.orderId)-payment.payAmount);
	    		if(StructUtils.tmallCacheMap.get(payment.orderId)<0.01){
	    			StructUtils.tmallCacheMap.remove(payment.orderId);
	    		}
	    	}else{
	    		System.err.println("      Current TaobaoMap、TmallMap size is "
						+ StructUtils.taobaoCacheMap.size() + "  ,  " + StructUtils.tmallCacheMap.size()+"	===========pay is faster than order	!!!============");
	    	}
	    }
//	    collector.ack(input);
	}
